...
class virtual base_visitor =
  object
    ...
    method virtual visitStorageLocation
        : Solidity_ast.storage_location -> action
    ...
    method virtual visitCompareOperator
        : Solidity_ast.compare_operator -> action
  end

(* To include in every visitor to avoid defining every method *)
class init_base_visitor =
  object
...
let visitOpt (visitElt : #base_visitor -> 'a -> unit) (v : #base_visitor) :
    'a option -> unit =
  let continueVisit = function
    | None -> ()
    | Some elt -> visitElt v elt
  in
  handleAction v#visitOption continueVisit

let visitList (visitElt : #base_visitor -> 'a -> unit) (v : #base_visitor) :
    'a list -> unit =
  let continueVisit = List.iter (visitElt v) in
  handleAction v#visitList continueVisit

let visitXY (visitX : #base_visitor -> 'a -> unit)
    (visitY : #base_visitor -> 'b -> unit) (visitor : #base_visitor)
    ((x, y) : 'a * 'b) : unit =
  visitX visitor x;
  visitY visitor y

let visitXYZ (visitX : #base_visitor -> 'a -> unit)
    (visitY : #base_visitor -> 'b -> unit)
    (visitZ : #base_visitor -> 'c -> unit) (visitor : #base_visitor)
    ((x, y, z) : 'a * 'b * 'c) : unit =
  visitX visitor x;
  visitY visitor y;
  visitZ visitor z
...
let visitNode (visitElt : (#ast_visitor as 'b) -> 'a -> unit) (v : 'b)
    (elt : 'a node) : unit =
...
let rec visitType (v : #ast_visitor) (t : Solidity_ast.type_) : unit =
  let continueVisit (t : Solidity_ast.type_) : unit =
    match t with
    | ElementaryType e -> visitElementaryType (v :> base_visitor) e
    | Array (t, e) ->
      visitType v t;
    ...
    | Mapping (tk, tv) ->
      visitType v tk;
      visitType v tv
    | FunctionType f -> visitFunctionType v f
    | UserDefinedType li -> visitNode visitLongIdent v li
    | Optional tk -> List.iter (visitType v) tk
  in
...
and visitFunctionType (v : #ast_visitor) (ft : Solidity_ast.function_type) :
    unit =
  let continueVisit
      ({ fun_type_params;
         fun_type_returns;
        ...
        Solidity_ast.function_type ) : unit =
    visitList visitParam v fun_type_params;
    visitList
      (visitXY visitType (visitOpt visitStorageLocation))
      v fun_type_returns;
    visitVisibility v fun_type_visibility;
    visitFunMutability v fun_type_mutability
  in
...
and visitInheritanceSpecifier (v : #ast_visitor)
    (is : Solidity_ast.inheritance_specifier) =
  visitXY (visitNode visitLongIdent) (visitList @@ visitExpression) v is

and visitContractPart (v : #ast_visitor) (cp : Solidity_ast.contract_part) :
    unit =
...
and visitStateVariableDef (v : #ast_visitor) (svd : state_variable_definition) :
    unit =
  let continueVisit
      ({ var_name;
         var_type;
...
and visitBlock (v : #ast_visitor) (b : block) : unit =
  visitList visitStatement v b
...
and visitFunctionDef (v : #ast_visitor) (fd : function_definition) : unit =
  let continueVisit
    ...
    visitList
      (visitXY (visitNode visitLongIdent)
    ...
    visitOpt visitBlock v fun_body
  in
  handleAction v#visitFunctionDef continueVisit fd

and visitFunctionDefNode (v : #ast_visitor) (fdn : function_definition node) :
    unit =
  visitNode visitFunctionDef v fdn

and visitModifierDef (v : #ast_visitor) (md : modifier_definition) : unit =
  let continueVisit
      ({ mod_name; mod_params; mod_override; mod_virtual; mod_body } :
    ...
    visitOpt visitBlock v mod_body
  in
  handleAction v#visitModifierDef continueVisit md

and visitModifierDefNode (v : #ast_visitor) (mdn : modifier_definition node) :
    unit =
  visitNode visitModifierDef v mdn

and visitEventDef (v : #ast_visitor) (ed : event_definition) : unit =
  let continueVisit
      ({ event_name; event_params; event_anonymous } : event_definition) : unit
      =
    visitNode visitIdent v event_name;
    visitList
      (visitXYZ visitType visitBool (visitOpt (visitNode visitIdent)))
      v event_params;
...
and visitCatchClause (v : #ast_visitor) (cc : catch_clause) : unit =
  visitXYZ
    (visitOpt (visitNode visitIdent))
    (visitList visitParam) visitBlock v cc
...
and visitExpression (v : #ast_visitor) (e : expression) : unit =
  let continueVisit (e : expression) : unit =
    ...
    | ArraySlice (e, eo1, eo2) ->
      ...
      visitOpt visitExpression v eo1;
      visitOpt visitExpression v eo2
    | TupleExpression eol -> visitList (visitOpt visitExpression) v eol
    | PrefixExpression (u, e) ->
      visitUnaryOperator v u;
      visitExpression v e
    | SuffixExpression (e, u) ->
      visitExpression v e;
      visitUnaryOperator v u
...
and visitFunctionCallArguments (v : #ast_visitor) (fca : function_call_arguments)
    : unit =
...
and visitSourceUnit (v : #ast_visitor) (su : source_unit) : unit =
  let continueVisit (su : source_unit) : unit =
    ...
    | Pragma (i, s) ->
      visitIdent v i;
      visitString v s
    | Import id -> visitImportDirective v id
...
and visitImportDirective (v : #ast_visitor) (id : import_directive) : unit =
  let continueVisit
      ({ import_from; import_symbols; import_pos = _ } : import_directive) :
      unit =
    visitString v import_from;
    visitImportSymbols v import_symbols
  in
  handleAction v#visitImportDirective continueVisit id

and visitImportSymbols (v : #ast_visitor) (is : import_symbols) : unit =
  let continueVisit = function
    | ImportAll io -> visitOpt (visitNode visitIdent) v io
    | ImportIdents l ->
      visitList
        (visitXY (visitNode visitIdent) (visitOpt (visitNode visitIdent)))
        v l
  in
