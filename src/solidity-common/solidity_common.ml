...
module LongIdent = struct
  ...
  let equal lid1 lid2 = compare lid1 lid2 = 0
  ...
  let printf fmt lid = Format.fprintf fmt "%s" (to_string lid)
end
...
module IdentAList = struct
  ...
  let map f list = List.map (fun (key, v) -> (key, f v)) list
...
module ExtList = struct
  ...
  let for_all2 f l1 l2 = same_lengths l1 l2 && List.for_all2 f l1 l2
...
module ExtZ = struct
  ...
  let is_neg z = Z.sign z < 0

  let is_pos z = Z.sign z >= 0
...
module ExtQ = struct
  let is_int q = Z.equal Z.one (Q.den q)

  let is_neg q = Q.sign q < 0

  let is_pos q = Q.sign q >= 0
...
let annot contents annot pos = { contents; annot; pos }

let strip n = n.contents

let get_annot n = n.annot
...
let string_of_pos (file, pos1, pos2) =
  let source =
    ...
    | _ -> ""
  in
  ( Printf.sprintf "%s:%d.%d-%d.%d" file (fst pos1)
      (snd pos1 + 1)
      (fst pos2)
      (snd pos2 + 1),
    source )
...
let prim_of_id id : primitive option = prim_by_id.(id)

let prim_of_ident prim : (int * primitive) option =
  StringMap.find_opt (Ident.to_string prim) !prim_by_name

let add_primitive id p =
  if id < 0 then error "Negative primitive id";
  if id > max_primitives then error "Primitive id above limit";
  begin
  ...
  if id > !max_prim_id then max_prim_id := id
