...
exception
  ForbiddenCall of Ident.t * fun_mutability * Ident.t * fun_mutability * pos
...
let invariant_broken s = raise (InvariantBroken s)

let type_error pos fmt =
  Format.kasprintf (fun s -> raise (TypecheckError (s, pos))) fmt

let string_of_exn = function
  | Solidity_common.GenericError s -> Printf.sprintf "Generic error: %s" s
  | SyntaxError (s, pos) ->
  ...
  | TypecheckError (s, pos) ->
    let loc, source = Solidity_common.string_of_pos pos in
    Printf.sprintf "%s:\nType error: %s\n%s" loc s source
  | ForbiddenCall (id1, mut1, id2, mut2, pos) ->
    let loc, source = Solidity_common.string_of_pos pos in
