...
let disable_postcheck () = postcheck := false
  ...
  let arg_list =
    Arg.align
      [ ( "--version",
          Arg.Unit
            (fun () ->
              Format.eprintf "Solidity Parser & Typechecker";
              exit 0 ),
          " Show version and exit" );
        ( "--no-typecheck",
          Arg.Unit disable_typecheck,
          " Disable the typechecker" );
        ("--freeton", Arg.Set freeton, " Parse for freeton");
        ( "--no-postcheck",
          Arg.Unit disable_postcheck,
  ...
  let arg_usage =
    String.concat "\n"
      [ Filename.basename Sys.executable_name ^ " [OPTIONS] FILE [OPTIONS]";
        "";
        "This tool will parse and typecheck a Solidity file (.sol) and print \
         the result";
        "";
        "Available options:"
      ]
