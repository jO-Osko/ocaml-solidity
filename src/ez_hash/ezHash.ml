...
*)
  ...
  external blake2b_init_key : bytes -> int -> string -> blake2b_ctx
    = "blake2b_init_key_ml"
  ...
  let blake2b_init ?key ?(size = 64) () =
    ...
    let ctx =
      ...
      | Some key -> blake2b_init_key ctx size key
    in
    (ctx, hash)
  ...
  let () =
    ...
    assert (
      test_BLAKE2b_512 "The quick brown fox jumps over the lazy dog"
...
*)
  ...
  let () =
    let test_SHA256 input = Hex.encode (sha256 input) in
    assert (
      test_SHA256 ""
  ...
  let () =
    let test_KEC input = Hex.encode (sha3 KEC256 input) in
    assert (
      test_KEC ""
      = "c5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470" );

    assert (
      test_KEC "The quick brown fox jumps over the lazy dog"
