...
let compute_selector pos ~library id args =
  ...
  in
  let fun_sig =
    Format.asprintf "%a(%s)" Ident.printf id (String.concat "," arg_types)
  in
...
let eval_array_length_exp env e =
  ...
  let rec aux e =
    match strip e with
    | NumberLiteral (q, unit, _hex_size_opt) -> apply_unit q unit
  ...
  and process_ident id iddl =
    match iddl with
    | [ Variable
          { variable_mutability = MConstant;
            variable_def = Some { var_init = Some e; _ };
            _
          }
      ] ->
      aux e
    | _ -> error id.pos "Undeclared identifier: %a" Ident.printf (strip id)
  in
...
let type_desc_to_base_type ~loc : type_desc -> type_ = function
  | TDEnum ed -> TEnum (ed.enum_abs_name, ed)
  | TDStruct sd ->
...
let elementary_type_to_type ~loc : Solidity_ast.elementary_type -> type_ =
  ...
  | TypeBool -> TBool
  | TypeInt sz -> TInt sz
  | TypeUint sz -> TUint sz
  | TypeFixed (sz, dec) -> TFixed (sz, dec)
  | TypeUfixed (sz, dec) -> TUfixed (sz, dec)
  | TypeAddress payable -> TAddress payable
  | TypeBytes (Some sz) -> TFixBytes sz
  | TypeBytes None -> TBytes loc
  | TypeString -> TString loc
  ...
  | TypeAbstract s ->
    Printf.kprintf failwith
      "Unknown abstract type %S in \
       Solidity_type_builder.elementary_type_to_type"
      s

let rec ast_type_to_type pos ~loc env = function
  | ElementaryType et -> elementary_type_to_type ~loc et
  | UserDefinedType lid ->
    let t =
      match Solidity_tenv.find_type env (strip lid) with
      | Some t -> Solidity_type.change_type_location loc t
      | None ->
  ...
  | Optional tk ->
    TOptional
      ...
      | _ ->
        TTuple (List.map (fun t -> Some (ast_type_to_type pos ~loc env t)) tk)
      )
  | Mapping (tk, tv) -> (
    match loc with
    | LMemory
    | LCalldata
      when not !for_freeton ->
      error pos
        "Mapping types can only have a data location of \"storage\" and thus \
         only be parameters or return variables for internal or library \
         functions"
    | _ ->
      (* Mapping keys are always in memory
         Mapping values are always in storage *)
      TMapping
        ( ast_type_to_type pos ~loc:LMemory env tk,
...
and var_type_to_type pos env ~arg ~ext loc_opt t =
  let loc =
    storage_location_to_location (Option.value ~default:Memory loc_opt)
  in
  let t = ast_type_to_type pos ~loc env t in
  ( if not !for_freeton then
    if Solidity_type.is_reference_type t then begin
      let valid =
        match loc_opt with
        | None -> false
      ...
      if not valid then
        ...
        in
        error pos "Data location must be %s for %s, but %s was given" expected
          context given
      else if
        Solidity_type.has_mapping t && not (Solidity_type.is_storage_type t)
      then
        error pos
          "Type %s is only valid in storage because it contains a (nested) \
           mapping"
...
and function_type_to_desc pos env ft =
  ...
  let function_returns =
    process_fun_type_returns pos env ~ext ft.fun_type_returns
  in
...
and process_fun_type_returns pos env ~ext returns =
  List.map
    (fun (t, loc_opt) ->
      (var_type_to_type pos env ~arg:true ~ext loc_opt t, None) )
    returns

let process_event_params pos env params =
  List.map
    (fun (t, _indexed, name_opt) ->
      (ast_type_to_type pos ~loc:LMemory env t, Option.map strip name_opt) )
    params
...
let variable_type_to_function_type loc t =
  ...
  let rec aux atl = function
    | TArray (t, _sz_opt, _loc) -> aux ((TUint 256, None) :: atl) t
    | TMapping (kt, vt, _loc) -> aux ((kt, None) :: atl) vt
    | TStruct (_lid, sd, _loc) ->
      let rtl =
        List.map
          (fun (_id, t) ->
            if allowed_in_struct t then
              (t, None)
            else
              error loc "Can not make getter for such type" )
          sd.struct_fields
      in
      (List.rev atl, rtl)
    | t -> (List.rev atl, [ (t, None) ])
  in
  aux [] t

(* Make a getter function from a variable *)
let variable_desc_to_function_desc pos vid variable_abs_name vt : function_desc
    =
  let function_params, function_returns =
    variable_type_to_function_type pos vt
  in
  let function_selector =
    Some (compute_selector pos ~library:false vid function_params)
  in
...
let process_overrides env overrides_opt =
  Option.map
    (List.map (fun lid_node ->
         ...
         | None ->
           error lid_node.pos "Invalid contract specified in override list: %a"
             LongIdent.printf lid
         | Some cd -> cd.contract_abs_name ) )
    overrides_opt

let make_modifier_desc mlid md =
  { modifier_abs_name = mlid; modifier_params = []; modifier_def = md }
...
let make_event_desc elid ed =
  { event_abs_name = elid; event_params = []; event_def = ed }
...
let update_variable_desc pos env vd kind_opt =
  ...
  in
  let variable_type =
    ast_type_to_type pos ~loc:(LStorage false) env vd'.var_type
  in
  let variable_getter =
    match (vd'.var_visibility, kind_opt) with
    | VPublic, Some (Solidity_ast.Contract | Interface) ->
      Some
        (variable_desc_to_function_desc pos (strip vd'.var_name)
...
let update_function_desc pos env fd kind_opt =
  ...
  let function_selector =
    if not ext then
      ...
      Some (compute_selector pos ~library fid function_params)
  in
...
let update_struct_fields sd fields =
  let fields =
    List.fold_left
      (fun fields (field_name, field_type) ->
...
let primitive_fun_desc ?(returns_lvalue = false) ?(purity = PurityPure)
...
let primitive_fun_type ?(kind = KOther) ?(returns_lvalue = false) arg_types
    ret_types function_mutability =
  let fd =
    primitive_fun_desc ~returns_lvalue arg_types ret_types function_mutability
  in
  TFunction (fd, { new_fun_options with kind })

let primitive_fun ?(returns_lvalue = false) ?purity arg_types ret_types
    function_mutability =
  let fd =
    primitive_fun_desc ~returns_lvalue ?purity arg_types ret_types
      function_mutability
  in
  Function fd
