...
let binop_type pos op t1 t2 : type_ =
  ...
  | (BExp | BLShift | BRShift), _, TRationalConst (q2, _) ->
    if not (ExtQ.is_int q2) then error_incompat ();
  ...
  | (BPlus | BMinus | BTimes | BDiv | BMod | BAnd | BOr | BXor), _, _ -> (
    ...
    | _ -> error_incompat () )
  | (BLAnd | BLOr), TBool, TBool -> TBool
  | (BLAnd | BLOr), _, _ -> error_incompat ()
...
let unop_type pos op t : type_ =
  ...
  in
  match (t, op) with
  | _, UPlus -> error pos "Usage of unary + is disallowed"
  | TRationalConst (q, _), _ -> (
    match apply_unop op q with
    | Some q -> TRationalConst (q, None)
    | None -> error_incompat () )
  | ( ( TBool | TInt _ | TUint _ | TFixed _ | TUfixed _ | TAddress _
      | TFixBytes _ | TBytes _ | TString _ | TEnum _ | TStruct _ | TContract _
      | TArray _ | TFunction _ ),
      UDelete ) ->
    TTuple []
  | _, UDelete -> error_incompat ()
...
let immediate_array_element_type pos tl : type_ =
  ...
  let rec aux t = function
    ...
    | t' :: tl ->
      let t =
        if
          Solidity_type_conv.implicitly_convertible ~ignore_loc:true ~from:t'
            ~to_:t ()
        then
          t
        else
  ...
  | t :: tl -> (
    ...
    | ( TTuple _ | TArraySlice _ | TFunction _ | TModifier _ | TEvent _
      | TType _ | TMagic _ ) as t ->
...
let check_argument_type pos kind ~expected ~provided =
  if
    not
      (Solidity_type_conv.implicitly_convertible ~from:provided ~to_:expected ())
  then
    error pos
      "Invalid type for argument in %S. Invalid implicit conversion from %S to \
       %S requested"
      kind
...
let check_function_application pos kind fp args =
  ...
  in
  if
    nb_args_provided <> nb_args_expected
    && ((not has_dots) || nb_args_expected > nb_args_provided)
  then
    error pos "Wrong argument count for %S: %d arguments given but expected %d"
      ...
      List.iter
        (fun (fid, at) ->
          ...
          | Some (ft, _id_opt) ->
            check_argument_type pos kind ~expected:ft ~provided:at
          | None ->
            error pos "Named argument %S does not match function declaration"
              (Ident.to_string fid) )
        ntl
...
let using_for_allowed base_t_opt =
  ...
  | None
  | Some
      ( TModifier _ | TEvent _ | TTuple _ | TArraySlice _ | TType _ | TMagic _
...
let is_contract_instance base_t_opt =
  match base_t_opt with
  | Some (TContract (_lid, _cd, false (* super *))) -> true
  | Some _
  | None ->
    false
...
let fun_opt base_t_opt is_uf (fd : function_desc) =
  ...
  let kind =
    ...
    | VExternal
    | VPublic
      when is_contract_instance base_t_opt && not is_uf ->
      KExtContractFun
    | _ -> KOther
  in
...
let get_variable_getter pos vd =
  if vd.variable_local then error pos "Requesting getter on a local variable !";
...
let type_and_annot_of_id_desc pos opt base_t_opt idd is_uf =
  ...
  | Function fd when is_uf ->
    ...
    assert (not fd.function_is_primitive);
    (* just a check *)
    assert (not fd.function_is_method);
    (* = function from library *)
    let fd' =
      match fd.function_params with
      | (_ft, _id_opt) :: fp -> { fd with function_params = fp }
      | _ -> invariant_broken __LOC__
    in
    ( TFunction (fd', fun_opt base_t_opt true fd'),
      RightValue,
      AFunction (fd, true) )
  ...
  | Constr cd -> (cd.constr_type, RightValue, AConstr cd)
  | Module md -> (TModule (md.module_abs_name, md), RightValue, AModule md)
  | Alias _ad -> error pos "Alias should no longer be present at this point"
...
let resolve_overloads pos opt base_t_opt id iddl uf_iddl =
  ...
  let err_notfound, err_notunique, err_nomatch, err_manymatch =
    ...
    | None ->
      (* No result *)
      ( (fun () -> error pos "Undeclared identifier: %S" (Ident.to_string id)),
        ...
        (fun () ->
          error pos "No matching declaration found after variable lookup" ),
    ...
    | Some t ->
      ...
      let err_notfound () =
        error pos
          "Member %S not found or not visible after argument-dependent lookup \
           in %s"
          (Ident.to_string id)
          (Solidity_type_printer.string_of_type t)
      in
      ...
      let err_notunique () =
        error pos "Member %S not unique after argument-dependent lookup in %s"
          (Ident.to_string id)
          (Solidity_type_printer.string_of_type t)
      in
    ...
    let iddl =
      List.fold_left
        (fun iddl idd ->
          match idd with
          (* Function *)
          | Function fd -> add_if_compatible idd iddl fd args
          (* Variable getter *)
          | Variable vd when is_contract_instance base_t_opt ->
            let fd = get_variable_getter pos vd in
          ...
          | Variable vd -> (
            ...
            | TFunction (fd, _fo) -> add_if_compatible idd iddl fd args
        ...
        (fun uf_iddl idd ->
          ...
          | Function fd -> (
            ...
            | Some at, (ft, _id_opt) :: fp
              when Solidity_type_conv.implicitly_convertible ~from:at ~to_:ft ()
              ->
...
let type_ident opt env base_t_opt id_node =
  ...
  let iddl =
    match base_t_opt with
    | None -> Solidity_tenv.find_ident env ~lookup:LInternal id
    | Some base_t -> (
      match base_t with
      ...
      | TType (TContract (lid, cd, _super)) ->
        (* super should be false *)
        let is_parent = List.mem lid opt.current_hierarchy in
        let lookup =
          Solidity_tenv.LStatic (cd.contract_def.contract_kind, is_parent)
        in
      ...
      | TStruct (_lid, sd, _loc) -> (
        ...
        | Some t ->
          let fd =
            { field_struct_desc = sd; field_name = id; field_type = t }
          in
          [ Field fd ]
        | None -> [] )
      | _ -> [] )
  ...
  let uf_iddl =
    ...
    | Some t when using_for_allowed base_t_opt ->
      ...
      in
      let envs =
        AbsLongIdentMap.fold
          (fun _lid (env, tl) envs ->
            if
              tl = []
              || List.exists
                   (fun t' ->
                     Solidity_type_conv.implicitly_convertible ~from:t ~to_:t'
                       () )
                   tl
            then
              env :: envs
  ...
  let t, lv, a =
    type_and_annot_of_id_desc id_node.pos opt base_t_opt idd is_uf
  in
  set_annot id_node a;
  (t, lv)
...
and type_expression_lv opt env exp : type_ * value_kind =
  let pos = exp.pos in
  let t, lv =
    match strip exp with
    (* Literals *)
    | BooleanLiteral _b -> (TBool, RightValue)
      ...
      let sz_opt =
        ...
        | Some i ->
          if i mod 2 = 0 then
            ...
            None
          (* Note: if not even, size info is no longer relevant *)
        | None -> None
      in
      (TRationalConst (q, sz_opt), RightValue)
    | StringLiteral s -> (TLiteralString s, RightValue)
    | AddressLiteral _a ->
    ...
    | ArrayAccess (e, None) -> (
      ...
      | _ -> error pos "Index expression cannot be omitted" )
    | ArrayAccess (e1, Some e2) -> (
      ...
      | TType t -> (
        ...
        | None -> error pos "Integer constant expected" )
      ...
      | TString _loc -> error pos "Index access for string is not possible"
      | t ->
        error pos
    ...
    | ArraySlice (e1, e1_opt, e2_opt) -> (
      ...
      | TArraySlice (t, loc) ->
        Option.iter
          (fun e -> ignore (expect_array_index_type opt env None e))
          e1_opt;
        Option.iter
          (fun e -> ignore (expect_array_index_type opt env None e))
          e2_opt;
        ...
        not
          ...
          | TOptional t1 ->
            ...
            (* Printf.eprintf "convert %s <- %s\n%!"
               ( Solidity_type_printer.string_of_type t1)
               ( Solidity_type_printer.string_of_type t2); *)
            Solidity_type_conv.implicitly_convertible ~from:t2 ~to_:t1 ()
          | _ -> false )
      then
          ...
          (fun (tl, lv) e_opt ->
            ...
            | Some e -> (
              ...
              ( Some t :: tl,
                ...
                | _ -> RightValue ) )
            | None when opt.allow_empty -> (None :: tl, lv)
            | None -> error pos "Tuple component cannot be empty" )
          ([], LeftValue []) eol
      in
    ...
    | IfExpression (e_if, e_then, e_else) -> (
      ...
      match
        Solidity_type_conv.common_type
          (Solidity_type_conv.mobile_type pos t1)
          (Solidity_type_conv.mobile_type pos t2)
      with
      | Some t -> (t, RightValue)
      | None ->
        error pos
    ...
    | NewExpression t -> (
      ...
      | TString _ ->
        let t =
          Solidity_type_builder.primitive_fun_type [ TUint 256 ] [ t ] MPure
        in
        (t, RightValue)
      | TContract (_lid, cd, false (* super *)) ->
        ...
        let t =
          Solidity_type_builder.primitive_fun_type ~kind:KNewContract atl [ t ]
            MPayable
        in
        (t, RightValue)
      | TArray (_, Some _, _) ->
        error pos
          "Length has to be placed in parentheses after the array type for new \
           expression"
      | TStruct _
      | TEnum _ ->
        error pos "Identifier is not a contract"
      | _ -> error pos "Contract or array type expected" )
    | TypeExpression t ->
      ( TType (Solidity_type_builder.ast_type_to_type pos ~loc:LMemory env t),
        RightValue )
    | IdentifierExpression id_node -> type_ident opt env None id_node
    ...
    | FunctionCallExpression (e, args) -> (
      ...
      let t, lv = type_expression_lv { opt with call_args = Some args } env e in
      match (t, args) with
      (* Function call *)
      | TFunction (fd, _fo), args -> (
        check_function_application pos "function call" fd.function_params args;
            ...
            List.iter
              (function
                | AVariable (vd, _) -> (
                  ...
                  | None -> ()
                  | Some in_fd ->
                    in_fd.function_ops <- (vd, OpCall fd) :: in_fd.function_ops
                  )
                | _ -> () )
              list
        end;
        ...
        | [ (t, _id_opt) ] -> (t, lv_of_bool fd.function_returns_lvalue)
        | tl ->
      ...
      | TType t, AList [ at ] -> (
        ...
        replace_annot e (AType (TType t));
        match Solidity_type_conv.explicitly_convertible ~from:at ~to_:t with
        | Some t -> (t, RightValue)
        | None ->
          error pos "Explicit type conversion not allowed from \"%s\" to \"%s\""
            (Solidity_type_printer.string_of_type at)
            (Solidity_type_printer.string_of_type t) )
      | TType _, AList _ ->
        error pos "Exactly one argument expected for explicit type conversion"
      | TType _, ANamed _ ->
        error pos "Type conversion cannot allow named arguments"
      | ( ( TRationalConst _ | TLiteralString _ | TBool | TInt _ | TUint _
          | TFixed _ | TUfixed _ | TAddress _ | TFixBytes _ | TBytes _
          | TString _ | TEnum _ | TStruct _ | TContract _ | TArray _
          | TMapping _ | TTuple _ | TModifier _ | TArraySlice _ | TMagic _
          | TModule _ | TAbstract _ | TOptional _ | TAny | TDots ),
          _ ) ->
        error pos "Type is not callable" )
    | CallOptions
        ({ contents = IdentifierExpression { contents = id; _ }; _ }, opts)
    ...
    | CallOptions (e, opts) -> (
      ...
      | _ -> error pos "Expected callable expression before call options" )
  in
...
and type_function_args opt env args =
  match args with
  | ExpressionList el -> AList (List.map (type_expression opt env) el)
  | NameValueList nel ->
    let ntl =
      List.fold_left
        (fun ntl (id, e) ->
...
and expect_array_index_type opt env sz_opt exp =
  ...
  | TRationalConst (q, _), Some sz ->
    let n = Q.num q in
    if Z.equal (Z.min n sz) sz then error pos "Out of bounds array access";
    Some n
  | TRationalConst (q, _), None -> Some (Q.num q)
  | _ -> None
  ...
  List.fold_left
    (fun fo (id, e) ->
      ...
      let fo, already_set =
        ...
        | "value", KExtContractFun when (not !for_freeton) && not is_payable ->
          error pos "Cannot set option \"value\" on a non-payable function type"
        | "value", KNewContract when (not !for_freeton) && not is_payable ->
          error pos
            "Cannot set option \"value\", since the constructor of contract is \
             non-payable"
        ...
        | "gas", KNewContract ->
          error pos "Function call option \"%s\" cannot be used with \"new\""
            (Ident.to_string id)
        | "salt", KExtContractFun ->
          error pos "Function call option \"%s\" can only be used with \"new\""
            (Ident.to_string id)
        | _, KOther ->
          error pos
            "Function call options can only be set on external function calls \
             or contract creations"
            (Ident.to_string id)
        | _ ->
          error pos
            "Unknown option \"%s\". Valid options are \"salt\", \"value\" and \
             \"gas\""
            (Ident.to_string id)
      in
      if already_set then
        error pos "Option \"%s\" has already been set" (Ident.to_string id);
      fo )
    fo opts
...
let rec type_statement opt env s =
  ...
  | PlaceholderStatement ->
    if not opt.in_modifier then error pos "\"_\" has to be in a modifier"
  | ExpressionStatement e -> ignore (type_expression opt env e)
    ...
    begin
      let var_decl_list =
        List.map
          (fun var_decl_opt ->
            Option.map
              (fun (t, loc_opt, var_name) ->
                ( Solidity_type_builder.var_type_to_type pos env ~arg:false
                    ~ext:false loc_opt t,
                  var_name ) )
              var_decl_opt )
          var_decl_list
      ...
      let tl =
        ...
        | _ -> error pos "Type is not iterable"
      in
      if not (ExtList.same_lengths tl var_decl_list) then
        error pos
          "Left hand side and right hand side must have the same number of \
           elements"
      else
        List.iter2
          (fun var_decl_opt t ->
      ...
      List.iter
        (function
          | None -> ()
          | Some (t, var_name) ->
            Solidity_tenv_builder.add_local_variable pos env (strip var_name)
              (Solidity_type_builder.local_variable_desc t) )
        var_decl_list
    end;
    type_statement { opt with in_loop = true } env' s
  ...
  | TryStatement (e, returns, body, catch_clauses) ->
    ...
    let fd =
      match t with
      | TFunction (fd, { kind = KNewContract | KExtContractFun; _ }) -> fd
      | _ ->
        error pos
          "Try can only be used with external function calls and contract \
           creation calls"
    in
      ...
      | _ ->
        ...
        if nb_ret_fun <> nb_ret_clause then
          error pos
            ...
            nb_ret_fun nb_ret_clause;
        let returns =
          List.map
            (fun (t, loc_opt, name_opt) ->
              ( Solidity_type_builder.var_type_to_type pos env ~arg:true
                  ~ext:false loc_opt t,
                Option.map strip name_opt ) )
            returns
        in
        List.iter2
          (fun (rt, _id_opt1) (t, _id_opt2) ->
    ...
    List.iter
      (fun (t, var_opt) ->
        Option.iter
          (fun var_name ->
            Solidity_tenv_builder.add_local_variable pos env' var_name
              (Solidity_type_builder.local_variable_desc t) )
          var_opt )
      returns;
              ...
              (fun name ->
                let t =
                  Solidity_type_builder.var_type_to_type pos env ~arg:true
                    ~ext:false loc_opt t
                in
                Solidity_tenv_builder.add_local_variable pos env' (strip name)
                  (Solidity_type_builder.local_variable_desc t) )
              name_opt )
          params;
        List.iter (type_statement opt env') body )
  ...
  | Return (e, opts) -> (
    ...
    let fo = { Solidity_type_builder.new_fun_options with kind = KReturn } in
    let _fo = type_options opt env pos is_payable fo opts in
    set_annot s (AType annot);
    match (e, opt.fun_returns, opt.in_modifier) with
    | None, [], _ -> ()
    | None, _ :: _, _ -> error pos "Return arguments required"
    | Some _, _, true -> error pos "Return arguments not allowed"
    | Some e, [ rt ], _ -> (
      try expect_expression_type opt env e rt with
      | Failure s -> error pos "%s in return" s )
    | Some e, rtl, _ -> (
      try
        expect_expression_type opt env e (TTuple (List.map Option.some rtl))
      with
      | Failure s -> error pos "%s in return" s ) )
  | VariableDefinition def ->
    let var_decl_list =
      ...
      | VarInfer (var_name_list, e) ->
        let tl =
          match Solidity_type.unpromote_type (type_expression opt env e) with
          | TTuple tl ->
      ...
      | VarType (var_decl_list, exp_opt) ->
        ...
        Option.iter
          (fun e ->
            ...
            if not (ExtList.same_lengths tl var_decl_list) then
              error pos
                "Left hand side and right hand side must have the same number \
                 of elements"
            else
              List.iter2
                (fun var_decl_opt t ->
                  Option.iter
                    (fun (t', _var_name) ->
                      if
                        not
                          (Solidity_type_conv.implicitly_convertible ~from:t
                             ~to_:t' () )
                      then
                        error pos
                          "Incompatible types in assignment: %S expected, got \
                           %S"
                          (Solidity_type_printer.string_of_type t')
                          (Solidity_type_printer.string_of_type t) )
                    var_decl_opt )
...
let constructor_params env lid =
  ...
  | [ Contract cd ] ->
    ...
    (* already set *)
    fd.function_params
  | [ _ ] -> error lid.pos "Contract expected"
  | _ :: _ :: _ -> error lid.pos "Multiple definitions found for contract !"
...
let modifier_or_constructor_params ~constructor env lid =
  ...
  | [ _ ] ->
    error lid.pos
      "Referenced declaration is neither a modifier nor a base contract"
  | _ :: _ :: _ ->
...
let typecheck_function_body pos opt cenv id params returns modifiers block =
  ...
  List.iter
    (fun (lid, el_opt) ->
      let params, is_constr =
        modifier_or_constructor_params ~constructor env lid
      in
      let args =
        match el_opt with
        | None -> []
...
let typecheck_contract_code pos (cd : contract_desc) =
  ...
  List.iter
    (fun (lid, el) ->
      ...
      | _ ->
        ...
        check_function_application lid.pos "constructor call" params (AList args)
      )
  ...
  IdentMap.iter
    (fun id iddl ->
      List.iter
        (fun (id_desc, id_origin) ->
          if not (Solidity_tenv.is_inherited id_origin) then
            match id_desc with
            | Variable ({ variable_def = Some { var_init = Some e; _ }; _ } as v)
              ->
            ...
            | Function
              ...
              let opt =
                { opt with
                  in_function = Some fd;
                  fun_returns = List.map fst fd.function_returns
                }
              in
              typecheck_function_body pos opt cenv id fd.function_params
                fd.function_returns fun_modifiers body
            | Function _ -> () (* Note: either no body or a builtin function *)
            | Modifier ({ modifier_def = { mod_body = Some body; _ }; _ } as md)
              ->
              let opt = { opt with in_modifier = true } in
              typecheck_function_body pos opt cenv id md.modifier_params [] []
                body
            | Modifier _ -> () (* Note: no body *)
            | Type _
            | Event _
            | Contract _ ->
              ()
...
let typecheck_free_function_code pos menv (fd : function_desc) =
  ...
  | Some { fun_name; fun_body = Some body; fun_modifiers; _ } ->
    ...
    in
    typecheck_function_body pos opt menv (strip fun_name) fd.function_params
      fd.function_returns fun_modifiers body
  | _ -> ()
        ...
        List.iter
          (fun part_node ->
            ...
            | UsingForDeclaration (lid_node, type_opt) -> (
              ...
              | None -> error pos "Identifier not found or not unique"
                  ...
                  Option.map
                    (fun t ->
                      Solidity_type_builder.ast_type_to_type pos
                        ~loc:(LStorage true) cenv t )
                    type_opt
                in
                Solidity_tenv_builder.add_using_for cenv lib type_opt )
            | _ -> () )
          cd.contract_parts;
        if (not cd.contract_abstract) && not (is_interface cd.contract_kind)
        then
          ...
          | Some fid ->
            error cd.contract_name.pos
              "Contract %s should be marked as abstract because %s is virtual"
        ...
        (fun (idd, id_origin) ->
          ...
          | Defined -> (
            ...
            | Module _md ->
              ...
              ()
            | Contract cd -> postprocess_structs f cd.contract_env
            | Type (TDStruct sd) -> f sd
            | Type (TDEnum _)
            | Event _
            | Modifier _
            | Variable _
...
let check_struct_acyclicity env =
  let rec check_struct seen sd =
    if AbsLongIdentSet.mem sd.struct_abs_name seen then
      error (fst sd.struct_def).pos "Type definition %s is cyclic"
  ...
  and check_type seen t =
    match t with
    | TStruct (_lid, sd, _) -> check_struct seen sd
    | TArray (t, Some _, _) -> check_type seen t
    | _ -> ()
  in
  ...
  List.iter
    (fun part_node ->
      ...
      | TypeDefinition td ->
        ...
        Solidity_tenv_builder.add_contract_ident cd (strip tid_node) (Type td');
      ...
      | StateVariableDeclaration vd ->
        ...
        begin
          ...
          | Interface -> error pos "Variables can not be declared in interfaces"
          | _ -> ()
        end;
        if is_external vd.var_visibility then
          error pos "Variable visibility set to external";
        if is_constant vd.var_mutability && is_none vd.var_init then
      ...
      | FunctionDefinition fd ->
        ...
        begin
          ...
          | Interface ->
            ...
            if is_some fd.fun_body then
              error pos "Functions in interfaces cannot have an implementation";
        ...
        if is_construct then begin
          if is_none fd.fun_body then
            error pos "Constructor must be implemented if declared";
          if fd.fun_virtual then error pos "Constructors cannot be virtual";
          if (not !for_freeton) && (is_private || is_external) then
          ...
          if is_internal && not contract_abstract then
            error pos
              "Non-abstract contracts cannot have internal constructors. \
               Remove the \"internal\" keyword and make the contract abstract \
               to fix this";
          if
            (not !for_freeton)
            && not
                 ( is_payable fd.fun_mutability
                 || is_nonpayable fd.fun_mutability )
          then
            error pos
              "Constructor must be payable or non-payable, but is \"%s\""
              (Solidity_printer.string_of_fun_mutability fd.fun_mutability)
        end;
        if is_fallback then begin
          if not is_external then
        ...
        if is_receive then begin
          if not is_external then
            error pos "Receive ether function must be defined as \"external\"";
          if
            (not !for_freeton) && is_receive
            && not (is_payable fd.fun_mutability)
          then
            error pos "Receive ether function must be payable, but is \"%s\""
              (Solidity_printer.string_of_fun_mutability fd.fun_mutability)
        end;
        let fd, method_ =
          match contract_kind with
      ...
      | ModifierDefinition md ->
        ...
        if is_none md.mod_body && not md.mod_virtual then
          error pos "Modifiers without implementation must be marked virtual";
        begin
          match contract_kind with
          | Library ->
            ...
            if is_none md.mod_body then
              error pos "Modifiers in a library must be implemented if declared"
          | Interface -> ()
          | Contract -> ()
        end;
...
let preprocess_contract_inheritance menv cd =
  ...
  | Interface ->
    if cd.contract_abstract then
      error cid_node.pos
        "Interfaces do not need the \"abstract\" keyword, they are abstract \
         implicitly";
    List.iter
      (fun (lid_node, _el) ->
        let lid = strip lid_node in
        ...
        | None ->
          error lid_node.pos "Interface %s parent interface %s is undefined"
  ...
  | Contract ->
    List.iter
      (fun (lid_node, _el) ->
        ...
        match Solidity_tenv.find_contract menv lid with
        | None ->
          error lid_node.pos "Contract %s parent contract %s is undefined"
            (Ident.to_string cid) (LongIdent.to_string lid)
        | Some cd' ->
          set_annot lid_node (AContract cd');
          ensure_is_defined_before lid_node.pos cd';
...
let preprocess_free_function_definition menv (mlid : absolute LongIdent.t) fd =
  ...
  if fd.fun_virtual then error pos "Free functions can not be virtual";
  if is_some fd.fun_override then error pos "Free functions can not override";
  if is_none fd.fun_body then error pos "Free functions must be implemented";
...
let preprocess_free_variable_definition menv (mlid : absolute LongIdent.t) vd =
  ...
  if is_none vd.var_init then error pos "Uninitialized \"constant\" variable";
  ...
  List.iter
    (fun unit_node ->
      ...
      | Import { import_from; import_symbols; import_pos = _ } -> (
        ...
        | ImportAll None -> () (* Dealt with later *)
      ...
      | GlobalTypeDefinition td ->
        ...
        Solidity_tenv_builder.add_module_ident menv (strip tid_node) (Type td');
...
let rec resolve_imports_internal ~only_anonymous p seen ordered
    (m : Solidity_ast.module_) =
...
let type_program ?(init = Solidity_primitives.init) p =
  ...
  List.iter (fun m -> preprocess_module p menvs m) ordered_modules;
  ...
  (* Resolve all aliases *)
  List.iter
    (fun m -> Solidity_tenv_builder.resolve_aliases menvs m)
    ordered_modules;

  (* Check for name clashes in modules *)
  List.iter
    (fun m -> Solidity_tenv_builder.check_clashes_in_module menvs m)
    ordered_modules;
  ...
  List.iter (fun m -> postprocess_module_contracts m) ordered_modules;
...
let () = type_options_ref := type_options_fun
