...
let same_options o1 o2 =
  o1.value = o2.value && o1.gas = o2.gas && o1.salt = o2.salt
...
let rec same_type ?(ignore_loc = false) t1 t2 =
  ...
  | TUfixed (sz1, dec1), TUfixed (sz2, dec2) ->
    sz1 = sz2 && dec1 = dec2
  | TAddress payable1, TAddress payable2 -> payable1 = payable2
  | TFixBytes sz1, TFixBytes sz2 -> sz1 = sz2
  | TBytes loc1, TBytes loc2
  | TString loc1, TString loc2 ->
    ignore_loc || same_location loc1 loc2
  | TEnum (lid1, _ed1), TEnum (lid2, _ed2) -> LongIdent.equal lid1 lid2
  | TStruct (lid1, _ed1, loc1), TStruct (lid2, _ed2, loc2) ->
    LongIdent.equal lid1 lid2 && (ignore_loc || same_location loc1 loc2)
  ...
  | TArray (t1, None, loc1), TArray (t2, None, loc2) ->
    same_type ~ignore_loc t1 t2 && (ignore_loc || same_location loc1 loc2)
  ...
  | TMapping (src1, dst1, loc1), TMapping (src2, dst2, loc2) ->
    ...
    && (ignore_loc || same_location loc1 loc2)
  | TFunction (fd1, fo1), TFunction (fd2, fo2) ->
  ...
  | TArraySlice (t1, loc1), TArraySlice (t2, loc2) ->
    same_type ~ignore_loc t1 t2 && (ignore_loc || same_location loc1 loc2)
  | TType t1, TType t2 -> same_type ~ignore_loc t1 t2
  | TMagic mt1, TMagic mt2 -> same_magic_type ~ignore_loc mt1 mt2
  | TModule (lid1, md1), TModule (lid2, md2) ->
    if LongIdent.equal lid1 lid2 then begin
  ...
  | TRationalConst (q1, sz_opt1), TRationalConst (q2, sz_opt2) -> (
    ...
    | _ -> false )
  | TLiteralString s1, TLiteralString s2 -> String.equal s1 s2
  | TTuple tl1, TTuple tl2 -> same_type_ol ~ignore_loc tl1 tl2
...
and same_magic_type ?(ignore_loc = false) t1 t2 =
  match (t1, t2) with
  | TMetaType t1, TMetaType t2 -> same_type ~ignore_loc t1 t2
  | TMsg, TMsg
  | TBlock, TBlock
  | TTx, TTx
  | TAbi, TAbi ->
...
let promote_loc : location -> location = function
  | LMemory -> LMemory
  | LCalldata -> LCalldata
  | LStorage _ -> LStorage false

(* Turn storage ref to storage pointer *)
let unpromote_loc : location -> location = function
  | LMemory -> LMemory
  | LCalldata -> LCalldata
  | LStorage _ -> LStorage true

(* Turn a type's location from storage ref to storage pointer *)
let rec unpromote_type : type_ -> type_ = function
  | TString loc -> TString (unpromote_loc loc)
  | TBytes loc -> TBytes (unpromote_loc loc)
  | TStruct (lid, sd, loc) ->
    (* Note: fields always remain storage ref,
       even if struct becomes storage pointer *)
    TStruct (lid, sd, unpromote_loc loc)
  | TArray (t, sz_opt, loc) -> TArray (t, sz_opt, unpromote_loc loc)
  | TArraySlice (t, loc) -> TArraySlice (t, unpromote_loc loc)
  | TMapping (tk, tv, loc) -> TMapping (tk, tv, unpromote_loc loc)
  | TTuple tl -> TTuple (List.map (Option.map unpromote_type) tl)
  | t -> t
...
let change_type_location loc t =
  let rec aux seen loc t =
    match t with
    | TString _loc -> TString loc
    | TBytes _loc -> TBytes loc
    ...
    | TStruct (lid, sd, LStorage _) when is_storage loc -> TStruct (lid, sd, loc)
    | TStruct (lid, sd, _loc) ->
      let sd' =
        ...
        | None ->
          ...
          sd'.struct_fields <-
            List.map
              (fun (id, t) -> (id, aux seen (promote_loc loc) t))
              sd.struct_fields;
          sd'
      in
      TStruct (lid, sd', loc)
    ...
    | TArraySlice (t, _loc) -> TArraySlice (aux seen (promote_loc loc) t, loc)
    | TMapping (tk, tv, _loc) ->
      TMapping (tk, aux seen (promote_loc loc) tv, loc)
    | TTuple tl -> TTuple (List.map (Option.map (aux seen loc)) tl)
    | t -> t
  in
