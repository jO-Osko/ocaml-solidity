...
let checkDetails (env : contract_env) (_fun_name : Ident.t)
    (fundet : function_details) : unit =
  let () =
    (* Checking that written variables were not immutable *)
    IdentMap.iter
      (fun writ annots ->
        try
        ...
        with
        | Not_found -> ()
        (* The global is not yet declared, we will check that after *) )
      fundet.fd_details.ed_writ
  in
  ()

let checkNoRead (env : contract_env) (fname : Ident.t)
    (fdetails : function_details) =
  let () =
    match fdetails.fd_details.ed_read_access with
    | [] -> ()
    | pos :: _ -> raise (ForbiddenReadAccess pos)
  in
  let () =
    match
      IdentMap.find_first_opt (fun _ -> true) fdetails.fd_details.ed_read
    with
    | None -> ()
    | Some (g, (annot, pos) :: _) -> (
      match annot with
      ...
      | AType (TFunction _) ->
        ()
      | _ -> (
        (* Pure function can read globals iff these globals are not mutable *)
        match IdentMap.find_opt g env.env_glob with
        | None ->
          (* This global is not registered ; must be an imported constant *)
          ()
        | Some { gd_mutab = MMutable; _ } ->
          raise (PureFunctionReadsGlobal (fname, g, pos))
        | Some _ -> () ) )
    | _ -> invariant_broken "Empty binding in ed_read"
  in
  ()
...
let checkNoWrite (fname : Ident.t) (fdetails : function_details) =
  let () =
    ...
    | [] -> ()
    | pos :: _ -> raise (ForbiddenWritState pos)
  in
  let () =
    match
      IdentMap.find_first_opt (fun _ -> true) fdetails.fd_details.ed_writ
    with
    | None -> ()
    | Some (g, (_, pos) :: _) -> raise (ForbiddenGlobalWrite (fname, g, pos))
    | _ -> invariant_broken "Empty binding in ed_write"
  in
  ()

let stateVariableAnalyser ?current_position env svd annot =
  let () =
    (* Checking the variable has not been defined before. *)
    match getGlobal svd.var_name.contents env with
    | None -> ()
    | Some _ ->
      raise (VariableAlreadyDefined (svd.var_name.contents, svd.var_name.pos))
  in
  let gd_def, gd_updates =
    ...
    | Some e, _ ->
      ...
      (GDefined (ed, e), [ (e.annot, e.pos) ])
  in
  let new_env =
    { env with
      env_glob =
        IdentMap.add svd.var_name.contents
  ...
  let new_env =
    ...
    | VPublic ->
      let params, _ =
        Solidity_type_builder.variable_type_to_function_type svd.var_name.pos
          type_
      in
      let getter_details =
        getDetails new_env (Getter svd) params (the current_position)
      in
      let () = checkDetails new_env svd.var_name.contents getter_details in
      addFun svd.var_name params getter_details new_env
    | VPrivate
    | VInternal
    | VExternal ->
      new_env
...
let analyseContract (env : contract_env) (contract : contract_definition) =
  let visit =
    object (self)
      ...
      method! visitFunctionDef fd =
        let fd =
          ...
          | Library ->
            fd
          | Interface -> { fd with fun_virtual = true }
        in
        let params, selector =
          match self#getAnnot () with
          | Some (AFunction ({ function_params; function_selector; _ }, _)) ->
            let function_selector =
              Option.value ~default:"" function_selector
            in
            (function_params, function_selector)
          | _ -> invariant_broken "Function should have function annot"
        in
        let fundet =
          getDetails env (Method (fd, selector)) params (the (self#getPos ()))
        in
        let () = checkDetails env fd.fun_name.contents fundet in
        env <- addFun fd.fun_name params fundet env;
        SkipChildren
      ...
      method! visitModifierDef md =
        let () =
          ...
          if not there_is_a_placeholder then
            raise
              (MissingPlaceholderStatement
                 (md.mod_name.contents, md.mod_name.pos) )
        in
        let md =
          match contract.contract_kind with
          | Contract
          | Library ->
            md
          | Interface -> { md with mod_virtual = true }
        in
        let params =
          match self#getAnnot () with
          | Some (AModifier { modifier_params; _ }) -> modifier_params
          | _ -> invariant_broken "Modifier should have modifier annot"
        in
        let fundet =
          getDetails env (Modifier md) params (the (self#getPos ()))
        in
        let () = checkDetails env md.mod_name.contents fundet in
        let fd_purity =
          (* Inferring modifier real mutability *)
          let no_write () =
            try
              ignore @@ checkNoWrite md.mod_name.contents fundet;
              true
            with
            | ForbiddenWritState _
            | ForbiddenGlobalWrite _ ->
              false
          in
          let no_read () =
            try
              ignore @@ checkNoRead env md.mod_name.contents fundet;
              true
            with
            | ForbiddenReadAccess _
            | PureFunctionReadsGlobal _ ->
              false
          in
          if not (no_write ()) then
            ...
            MPure
        in
        let fundet = { fundet with fd_purity } in
        env <- addFun md.mod_name params fundet env;
        SkipChildren

      method! visitStateVariableDef svd =
        let annot = the (self#getAnnot ()) in
        let new_env =
          stateVariableAnalyser ?current_position:(self#getPos ()) env svd annot
        in
        env <- new_env;
        SkipChildren
    end
  in
  let () =
    Solidity_visitor.visitContractDef
      (visit :> Solidity_visitor.ast_visitor)
      contract
  in
  visit#getEnv ()
...
let checkWrittenVar ~(in_constructor : bool) ~(env : contract_env)
    (written : Ident.t) (a_list : ast_annot list) =
  ...
  | MImmutable -> (
    if not in_constructor then
      ...
      (* Checking it is written once *)
      match gd.gd_def with
      | GDefined (_, e) ->
        raise (ImmutableUpdatedTwice (written, e.pos, getPos a_list))
      | GDeclared -> (
  ...
  | MConstant -> raise (ConstantUpdated (written, getPos a_list))
  | MMutable -> ()

(* To call once on each function *)
let checkFun (env : contract_env) (f : Ident.t)
    ((_, fdet) : fun_params * function_details) =
  let in_constructor = Ident.equal Ident.constructor f in
  let () =
    (* Checking that written vars are not immutable or constants *)
    IdentMap.iter (checkWrittenVar ~in_constructor ~env) fdet.fd_details.ed_writ
  in
  ()
...
let checkUnreadVar ~(env : contract_env) (read : Ident.t)
    (a_list : ast_annot list) =
  let gd, _ =
    try the @@ getGlobal read env with
    | InvariantBroken s ->
      invariant_broken
...
let checkFlowFun =
  ...
  let addInCheckedFuns (checked_funs : fun_params list IdentMap.t)
      (f : fun_identity) =
    ...
    | Some l ->
      if List.exists (equalParams ~check_ident:true f.fi_params) l then
        ...
        IdentMap.add f.fi_name (f.fi_params :: l) checked_funs
  in
  fun ~(env : contract_env) (f : fun_identity) ->
    let rec loop (checked_funs : fun_params list IdentMap.t) (f : fun_identity)
        : fun_params list IdentMap.t =
      try
        ...
        | None ->
          (* A read function that is not in the environment is a primitive *)
          checked_funs
        | Some ((_, fun_details), _) ->
          ...
          IdentMap.fold
            (fun f l (checked : fun_params list IdentMap.t) ->
              List.fold_left
                (fun (checked : fun_params list IdentMap.t) (fi_params, _) ->
                  loop checked { fi_params; fi_name = f } )
                checked l )
            fun_details.fd_details.ed_fun_calls checked_funs
      with
      | AlreadyExists -> checked_funs
    in
  ...
  IdentMap.iter
    (fun glob gd ->
      ...
      | GDefined _, MConstant, [] ->
        invariant_broken
          "There should be at least 1 update if constant is defined"
      | GDefined (ad, _e), MConstant, [ _a' ] ->
        (* assert e.annot = a'; *)
      ...
      | GDefined (ad, _e), MImmutable, [ _a' ] ->
        (* assert e.annot = a'; *)
        ignore @@ checkDefExpression ~env ad
      | GDefined _, MImmutable, [] ->
        invariant_broken
          "There should be at least 1 update if immutable is defined"
...
let validConstantCalls env (calls : (fun_params * ast_annot) list IdentMap.t) =
  ...
  IdentMap.for_all
    (fun call l ->
      ...
      List.for_all
        (fun (args, (annot, _)) ->
          (is_allowed_primitive || isAllowedCall annot)
          &&
...
let checkConstantFlow (env : contract_env) =
  ...
  let rec loop (studied_constants : IdentSet.t)
      (* Variables studied from the beginning *)
        (challenged_constants : Ident.t list)
      (* Variables studied in this loop search *) (cst : Ident.t) =
    if List.mem cst challenged_constants then
      raise
        (ConstantCycle (removeListPrefix Ident.equal cst challenged_constants))
      ...
      | GDefined (ad, _) ->
        ...
        IdentMap.fold
          ...
          ad.ed_read studied_constants
  in
  let _studied =
    IdentMap.fold
      (fun c _ studied -> loop studied [] c)
      constants IdentSet.empty
  in
...
let getFunDeclarations =
  let union (l1 : (fun_params * IdentSet.t) list)
      (l2 : (fun_params * IdentSet.t) list) : (fun_params * IdentSet.t) list =
    let l2' =
      (* l2 + l1 common elements *)
      List.fold_left
        (fun acc (p2, s2) ->
    ...
    in
    let l1' =
      (* l1 - l2 common elements *)
      List.filter
        (fun (p, _) ->
          not
          @@ List.exists (fun (p', _) -> equalParams ~check_ident:true p p') l2
          )
        l1
    in
    l1' @ l2'
  in
  let add_if_not_mem (cname : Ident.t) (f : fun_identity)
      (map : (fun_params * IdentSet.t) list IdentMap.t) :
      (fun_params * IdentSet.t) list IdentMap.t =
    match IdentMap.find_opt f.fi_name map with
    | None ->
      IdentMap.add f.fi_name [ (f.fi_params, IdentSet.singleton cname) ] map
    | Some l ->
      let exists =
        List.exists
      ...
      if exists then
        ...
        IdentMap.add f.fi_name
          ((f.fi_params, IdentSet.singleton cname) :: l)
          map
  in

  fun (env : contract_env) : (fun_params * IdentSet.t) list IdentMap.t ->
    let rec loop (acc : (fun_params * IdentSet.t) list IdentMap.t) contract_env
        : (fun_params * IdentSet.t) list IdentMap.t =
      ...
      let branches =
        List.map
          (fun (c, _) ->
            ...
            let cname_env =
              the (List.assoc_opt cname contract_env.env_inherited)
            in
            let new_funs =
              IdentMap.fold
                (fun f l acc ->
                    ...
                    (fun acc (fi_params, fd) ->
                      if fd.fd_defined then
                        add_if_not_mem
                          ...
                          { fi_name = f; fi_params } acc
                      else
                        acc )
                    acc l )
                cname_env.env_funs acc
            in
            ...
            if IdentSet.cardinal set >= 2 then
              match
                getFun ~in_inherited:false
                  { fi_name = f; fi_params = params }
                  env
              with
              | None ->
                (* Inheriting 2 function without overriding them: error *)
                raise
                  (NoOverrideMultipleFunDefs
                     (set, env.env_ctr.contract_name.contents, f) )
              | Some _ -> () )
          l )
    fun_decl
...
let addTested (fi : fun_identity) (p : fun_mutability) (ft : funs_tested) :
    funs_tested =
  match IdentMap.find_opt fi.fi_name ft with
...
let checkPurity =
  ...
  let rec function_calls (funs_tested : funs_tested)
      (max_purity : fun_mutability) (fident : fun_identity)
      (fdetails : function_details) (env : contract_env) : funs_tested =
    let () =
      match fdetails.fd_purity with
      | MPure ->
        checkNoRead env fident.fi_name fdetails;
        checkNoWrite fident.fi_name fdetails
      | MView -> checkNoWrite fident.fi_name fdetails
      | _ -> ()
    in
    ...
    IdentMap.fold
      (fun fi_name l funs_tested ->
        List.fold_left
          (fun funs_tested (fi_params, (annot, pos)) ->
            let studied = { fi_name; fi_params } in
            match isTested studied funs_tested with
            | Some purity ->
              (* Already tested, checking valid purity *)
            ...
            | None -> (
              ...
              | None -> (
                ...
                match annot with
                | AType (TFunction ({ function_mutability; _ }, _))
                ...
                | ANew _ ->
                  if callablePurity max_purity MPayable then
                    ...
                    raise
                      (ForbiddenCall
                         (fident.fi_name, max_purity, fi_name, MPayable, pos) )
                | _a ->
                  invariant_broken
                    (Ident.to_string fi_name ^ " was expected to be a function")
                )
              | Some ((_, studied_details), _) ->
                if callablePurity max_purity studied_details.fd_purity then
                  function_calls funs_tested
                    (minPurity max_purity studied_details.fd_purity)
                    fident studied_details env
                else
                  raise
                    (ForbiddenCall
                       ( fident.fi_name,
                         ...
                         pos ) ) ) )
          funs_tested l )
      fdetails.fd_details.ed_fun_calls funs_tested
  in

  (* Checks the purity of modifiers *)
  let modifiers (whole_envs : env) (env : contract_env)
      (max_purity : fun_mutability) (mod_name : absolute LongIdent.t)
      ((mod_params, _mod_details) : fun_params * modifier_details) =
    match getFunInEnvs mod_name mod_params whole_envs with
    | None ->
      invariant_broken
        (Format.sprintf "Cannot find modifier %s"
           (LongIdent.to_string mod_name) )
    | Some (fident, fdetails) -> (
           ...
           (fun fi_name l funs_tested ->
             List.fold_left
               (fun funs_tested (fi_params, fdets) ->
                 function_calls funs_tested fdets.fd_purity
                   { fi_name; fi_params } fdets env )
               funs_tested l )
           env.env_funs IdentMap.empty
    in
    let () =
      IdentMap.iter
        (fun _ ->
          List.iter (fun (_, { fd_modifiers; fd_purity; _ }) ->
              AbsLongIdentMap.iter
                (fun mod_name ->
                  List.iter (modifiers envs env fd_purity mod_name) )
                fd_modifiers ) )
        env.env_funs
    in
    ()
...
let checkModule (env : env) (m : module_) : env =
  let _, env =
    List.fold_left
      (fun (* 'globals' is a function building an empty environment with the global
              definitions ;
          ...
          let initial_env : contract_env =
            List.fold_left
              (fun (acc : contract_env) (id, _) ->
                if LongIdent.equal id name then
                  acc
                else
                  inheritFrom id acc env )
              (globals contract) cd.contract_hierarchy
          in
        ...
        | GlobalVariableDefinition svd, annot ->
          (* Updates the global function *)
          assertGlobalIsConstant svd;
          let globals contract =
            stateVariableAnalyser ~current_position:pos (globals contract) svd
              annot
          in
          (globals, env)
        | _ -> (globals, env) )
      (empty_contract_env, env) m.module_units
  in
  env
