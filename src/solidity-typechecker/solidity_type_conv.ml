...
let integer_part_size (size : int) (dec : int) = size - decimals_space dec
...
let rec implicitly_convertible ?(ignore_loc = false) ~from ~to_ () =
  match (from, to_) with
  | _, TAny -> true
  | TUint size1, TInt size2 -> size1 < size2
  ...
  | TRationalConst (q, _), TUint sz ->
    ExtQ.is_int q && (not (ExtQ.is_neg q)) && Z.numbits (Q.num q) <= sz
  | TRationalConst (q, _), TInt sz ->
    ...
    let n =
      if ExtQ.is_neg q then
        ...
        Q.num q
    in
    ExtQ.is_int q && Z.numbits n < sz
  | TLiteralString s, TString loc ->
    valid_utf8_string s
    && (ignore_loc || convertible_location ~from:LMemory ~to_:loc)
  | TLiteralString _s, TBytes loc ->
    ignore_loc || convertible_location ~from:LMemory ~to_:loc
  | TLiteralString s, TFixBytes sz -> String.length s <= sz
  | TAddress true, TAddress _payable -> true
  | TAddress false, TAddress _payable when !for_freeton -> true
  | TContract (_, derived, _), TContract (base, _, _) ->
    List.exists
      (fun (derived, _) -> LongIdent.equal derived base)
  ...
  | TMapping (tk1, tv1, loc1), TMapping (tk2, tv2, loc2) ->
    (ignore_loc || !for_freeton || (is_storage loc1 && is_storage_ptr loc2))
  ...
  | t1, TOptional t2 -> implicitly_convertible ~ignore_loc ~from:t1 ~to_:t2 ()
  ...
  | _ -> Solidity_type.same_type from to_
...
let rec explicitly_convertible ~from ~to_ : type_ option =
  ...
  match (from, to_) with
  | (TInt isz | TUint isz), TFixBytes bsz -> if_true (bsz = isz / 8)
  | TAddress _, TFixBytes bsz -> if_true (bsz = 20 || bsz = 21)
  ...
  | TContract (_, cd, _), TAddress payable ->
    if not payable then
      ...
      let idl =
        Solidity_tenv.lookup_ident cd.contract_env ~upper:false ~lookup:LAny
          Ident.receive
      in
      let payable =
        List.exists
  ...
  | TContract (_, derived, _), TContract (base, _, _) ->
    if_true
      (List.exists
         (fun (derived, _) -> LongIdent.equal derived base)
         derived.contract_hierarchy )
  | (TInt _ | TUint _), TAddress false -> Some (TAddress true)
  ...
  | TLiteralString _s, TBytes (LMemory | LStorage false) -> Some to_
  | TLiteralString s, TFixBytes bsz -> if_true (String.length s <= bsz)
  | (TString loc1 | TBytes loc1), (TString loc2 | TBytes loc2) ->
  ...
  | TStruct (lid1, _, loc1), TStruct (lid2, _, loc2) ->
    if_true
      (convertible_location ~from:loc1 ~to_:loc2 && LongIdent.equal lid1 lid2)
  ...
  | _ -> if_true (Solidity_type.same_type from to_)
...
let rec mobile_type pos t =
  ...
  | TRationalConst (q, _sz_opt) ->
    if ExtQ.is_int q then (
      ...
      if sz > 256 then error pos "Invalid rational number";
      if ExtQ.is_neg q then
        TInt sz
      else
        TUint sz
  ...
  | TTuple tl -> TTuple (List.map (Option.map (mobile_type pos)) tl)
  | _ -> t
