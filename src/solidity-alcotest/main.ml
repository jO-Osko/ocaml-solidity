...
let success f =
  ( f,
    fun () -> Alcotest.check_raises (spf "%s success" f) Success (check_file f)
  )

let fail f =
  (f, fun () -> Alcotest.check_raises (spf "%s fail" f) Fail (check_file f))
  ...
  let rec loop acc subdir : string list =
    ...
    Array.fold_left
      (fun acc file ->
        ...
        | Sys_error _s (* file is not a directory *) -> full_path_file :: acc )
      acc content
  in
  loop [] dir

let ok_files = List.map success (get_files success_dir)

let ko_files = List.map fail (get_files fail_dir)

let test_single_set =
  List.map
    (fun ((s, t) : string * (unit -> unit)) -> (spf "OK file %s" s, `Quick, t))
    ok_files
  @ List.map
...
(* Run it *)
let () = Alcotest.run "Solidity test suite" [ ("tests", test_single_set) ]
